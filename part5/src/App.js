...
function App() {
  ...
  async function handleUserRegister(e) {
    ...
    try {
      const registerReponse = await registerService.registerUser({ name, username: registerUsername, password: registerPassword })
  ...
  async function handleUserLogin(e) {
    ...
    try {
      const loginResponse = await registerService.loginUser({ username: loginUsername, password: loginPassword });
  ...
  const registerProps = { className: "formbox", handleUserRegister: handleUserRegister, name: name, 
  setName: setName, username: registerUsername, setUsername: setRegisterUsername, password: registerPassword, 
  setPassword: setRegisterPassword };

  const loginProps = { handleUserLogin: handleUserLogin, username: loginUsername, setUsername: setLoginUsername,
    password: loginPassword, setPassword: setLoginPassword };

  // const mainPageProps={ name: user.name, errorMessage: errorMessage, blogs: blogs }

  // const createBlogProps = { newBlog: newBlog, setNewBlog: setNewBlog, createBlog: createBlog }

  return (
    <div className="App">
      <h1>Blog App</h1>
      { user === null 
        ? <Homepage errorMessage={ errorMessage } registerProps={ registerProps } 
            loginProps={ loginProps } />
        : <MainPage username={ user.username } errorMessage={ errorMessage } blogs={ blogs } 
            createBlog={ createBlog } logOut={ logOut } blogRef={ createBlogRef }
            blogLikes={ blogLikes } deleteBlog={ deleteBlog } /> 
      }
    </div>
  );
};
